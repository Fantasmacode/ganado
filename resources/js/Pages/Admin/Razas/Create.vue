			...
			<form @submit.prevent="submit">
				<div class="modal-content">
					<div class="modal-header">
						<h6 class="modal-title" id="ModalLabel">Registrar Raza</h6>
					...
					<div class="modal-body">
						<div class="form-group bmd-form-group" :class="{ 'has-danger': errors.nombre_raz }">
							<label for="c_nombre_raz">Nombre</label>
							<input type="text" class="form-control" id="c_nombre_raz" placeholder="Nombre de la raza" v-model="form.nombre_raz" autofocus>
...
export default {
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.post(this.route('razas.store'), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
