...
import ModalCreate from '@/Pages/Admin/Usuarios/Create'
import ModalEdit from '@/Pages/Admin/Usuarios/Edit'
...
export default {
	...
	methods: {
		...
		edit(data) {
			axios.get(this.route('usuarios.edit', data.id))
			.then(response => {
		...
		destroy(data) {
			this.data = data;
			this.options.url = 'usuarios.destroy';
			this.options.message = 'Está seguro de eliminar al usuario: ';
			...
			this.options.name = this.data.nombres_usuario + " " + this.data.apellidos_usuario;
